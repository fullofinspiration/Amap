...
public class LocationModeSourceActivity extends Activity implements LocationSource,
	...
	private void setUpMap() {
		...
		.icon(BitmapDescriptorFactory
				.defaultMarker(BitmapDescriptorFactory.HUE_RED)));
		aMap.addMarker(new MarkerOptions()
		.position(Constants.GUIYANG)
		.icon(BitmapDescriptorFactory
				.defaultMarker(BitmapDescriptorFactory.HUE_RED)));
		aMap.addMarker(new MarkerOptions()
		.position(Constants.ANSHUN)
		.icon(BitmapDescriptorFactory
				.defaultMarker(BitmapDescriptorFactory.HUE_RED)));
		aMap.addNavigateArrow(new NavigateArrowOptions().add(Constants.FANGHENG,
				Constants.BEIJING, Constants.CHENGDU,Constants.SHANGHAI,Constants.GUIYANG,Constants.ANSHUN).width(20));
		//将镜头定位到图形所在区域
		aMap.moveCamera(CameraUpdateFactory
				.newCameraPosition(new CameraPosition(new LatLng(39.9875,
						116.48047), 16f, 38.5f, 300)));
		aMap.addCircle(new CircleOptions().center(Constants.GUIYANG)
